...
function renderExchange(d){
  ...
  return '<div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:8px;">'
    ...
    '<div style="margin-top:10px;padding-top:8px;border-top:1px solid '+LIGHT+';display:grid;grid-template-columns:repeat(2,1fr);gap:1px 12px;font-size:11px;line-height:1.6;">'+
    vals.slice(0,14).reverse().map((v,i)=>{
      ...
      const sign=up?'+':'';
      const diff=v-prev;
      return '<span style="display:flex;justify-content:space-between;"><span style="color:var(--muted);font-size:10px;">'+dates[idx]+'</span><span style="font-weight:600;font-size:10px;color:'+(up?BLUE:'#94a3b8')+';">'+v.toFixed(4)+' <span style="font-size:8px;font-weight:400;">'+(idx>0?sign+diff.toFixed(4):'')+'</span></span></span>';
    }).join('')+
    '</div>';
}
