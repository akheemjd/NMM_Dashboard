...
function renderExchange(d){
  ...
  const W=340,H=150,pad={top:22,right:10,bottom:22,left:36};
  const pw=W-pad.left-pad.right,ph=H-pad.top-pad.bottom;
  const Y=v=>pad.top+ph-(v-minV)/range*ph*0.72;
  ...
  svg+='<line x1="'+pad.left+'" y1="'+yHi+'" x2="'+(W-pad.right)+'" y2="'+yHi+'" stroke="#dc2626" stroke-width="1.5" stroke-dasharray="4,2" opacity="0.6"/>';
  svg+='<line x1="'+pad.left+'" y1="'+yLo+'" x2="'+(W-pad.right)+'" y2="'+yLo+'" stroke="#16a34a" stroke-width="1.5" stroke-dasharray="4,2" opacity="0.6"/>';
  svg+='<text x="'+(pad.left+4)+'" y="'+(yHi-4)+'" text-anchor="start" font-size="7" fill="#dc2626" font-weight="600">7d high '+wkHigh.toFixed(4)+'</text>';
  svg+='<text x="'+(pad.left+4)+'" y="'+(yLo-4)+'" text-anchor="start" font-size="7" fill="#16a34a" font-weight="600">7d low '+wkLow.toFixed(4)+'</text>';
