...
.mmap{flex:1;height:420px;background:#1a1d21}.mlist{width:280px;overflow-y:auto;flex-shrink:0;font-size:0.75rem}
  ...
  <div class="grid">
    ...
    <div class="module standard" id="calc-card"><button class="share-btn" onclick="shareModule(&quot;calc-card&quot;)" title="Copy">&#9998;</button>
      ...
      <div class="cform">
        ...
        <div class="cfield"><label>To</label><select id="calc-to"><option value="VAN">Vancouver, BC</option><option value="YYZ" selected>Toronto, ON</option></select></div>
...
setTimeout(function(){{
  ...
  if(mc&&incList.length){{
    ...
    incList.forEach(function(inc,i){{
      ...
      markers.push(m);
      bounds.push([lat,lon]);
      h+='<div class=\"mitem\" onclick=\"map.setView(['+lat+','+lon+'],12);\" style=\"cursor:pointer\"><div class=\"mhwy\">'+hwy+' &middot; '+prov+'</div><div class=\"mdesc\">'+desc.substring(0,80)+'</div></div>';
    }});
  ...
  if(md2&&(thInc.length||thHs.length)){{
    ...
    thInc.forEach(function(i){{
      ...
      L.circleMarker([i.lat,i.lng],{{radius:7,color:'#D93A34',fillColor:'#D93A34',fillOpacity:.5,weight:2}}).addTo(thMap).bindPopup('<b>'+i.title+'</b><br><span style="color:#D93A34;font-weight:600;">'+i.value+'</span><br>'+i.date+' &middot; '+i.location+'<br><b>Method:</b> '+i.method+'<br><b>Prevention:</b> '+i.prevention);
