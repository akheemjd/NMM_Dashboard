...
.card-footer{font-size:9px;color:var(--muted);margin-top:10px;padding-top:8px;border-top:1px solid var(--light)}
...
.inds{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:10px}
.ind{background:var(--bg);border-radius:8px;padding:12px 14px;display:flex;flex-direction:column;gap:3px}
.ind-label{font-size:9px;color:var(--muted);text-transform:uppercase;letter-spacing:.04em}
.ind-value{font-size:20px;font-weight:700;color:var(--blue)}
.ind-detail{font-size:11px;color:var(--muted)}
.ind-meaning{font-size:10px;color:var(--muted);line-height:1.3;margin-top:4px;padding-top:4px;border-top:1px solid var(--light)}
...
function renderMarket(d){
  ...
  d.indicators.forEach(i=>{
    ...
    h+='<div class="ind"><div style="display:flex;justify-content:space-between"><span class="ind-label">'+i.label+'</span><span style="color:'+ic+';font-weight:700">'+icon+'</span></div>';
...
function renderExchange(d){
  ...
  return '<div style="font-size:12px;color:var(--muted);margin-bottom:2px;">1 US Dollar equals</div>'
    ...
    +(d.updated?'<div class="card-footer">Updated '+ft(d.updated)+' &middot; Bank of Canada</div>':'');
}
...
function renderFuelSVG(d,type){
  ...
  return '<svg viewBox="0 0 '+W+' '+H+'" style="display:block;width:100%">'+svg+'</svg>'
    +(d.updated?'<div class="card-footer">Updated '+ft(d.updated)+'</div>':'');
}
...
function renderBorder(d){
  ...
  if(bl.length){h+='<div style="margin-top:10px;padding:8px 12px;background:#fef3c7;border-radius:6px;font-size:11px">';
    bl.forEach(b=>{const days=Math.ceil((new Date(b.date)-Date.now())/86400000);
      h+='<span style="color:var(--amber);font-weight:600">'+new Date(b.date).toLocaleDateString('en-CA',{month:'short',day:'numeric'})+'</span> &middot; '+b.name+' ('+days+'d)</div>';});
    h+='</div>';
  }
  return h;
}
