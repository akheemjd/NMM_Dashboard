...
<div class="banner">
  <nav style="padding-left:250px;font-size:15px;font-weight:700;font-family:'Fira Mono',monospace;">
    <a href="https://northernmilemedia.com" class="desk-link" style="color:var(--text);text-decoration:none;margin-right:18px;">Home</a>
    <a href="https://northernmilemedia.com/about/" class="desk-link" style="color:var(--text);text-decoration:none;">About</a>
...
<footer style="background:#0d1117;color:#8b949e;padding:18px 24px;margin-top:20px;font-size:11px;font-family:'SF Mono',monospace;line-height:1.5;display:flex;flex-wrap:wrap;gap:16px 40px;justify-content:space-between;align-items:center;border-top:1px solid #30363d;">
  <div style="display:flex;align-items:center;gap:10px;">
    <img src="logo.jpg" alt="Northern Mile Media" style="height:24px;width:auto;flex-shrink:0;">
    <div style="font-size:12px;font-weight:700;color:#c9d1d9;letter-spacing:.02em;">NORTHERN MILE MEDIA</div>
    <div style="font-size:9px;color:#8b949e;margin-left:auto;">&copy; 2026 Northern Mile Media &middot; Informational use only. Data from public sources. Verify independently.</div>
  </div>
...
function renderExchange(d){
  ...
  return '<div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:8px;">'
    ...
    // Daily rate list — last 14 days
    +'<div style="margin-top:10px;padding-top:8px;border-top:1px solid '+LIGHT+';">'
    +'<div style="font-size:9px;color:var(--muted);margin-bottom:4px;text-transform:uppercase;letter-spacing:.04em;">Daily Rates</div>'
    +'<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:1px 12px;font-size:11px;">'
    +vals.slice(0,14).reverse().map((v,i)=>{
      const idx=vals.length-1-i;
      const prev=idx>0?vals[idx-1]:v;
      const up=v>=prev;
      const sign=up?'+':'';
      const diff=v-prev;
      return '<div style="display:flex;justify-content:space-between;padding:1px 0;"><span style="color:var(--muted);">'+dates[idx]+'</span><span style="font-weight:600;color:'+(up?BLUE:'#94a3b8')+';">'+v.toFixed(4)+' <span style="font-size:8px;">'+(idx>0?sign+diff.toFixed(4):'')+'</span></span></div>';
    }).join('')
    +'</div></div>' 
}
